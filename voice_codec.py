...
# Codec2 frame geometry is queried per mode from the codec object, NOT hardcoded
# (700C = 320 samples/40 ms/4 B @ 700 bps; 3200 = 160 samples/20 ms/8 B @ 3200
# bps). The chosen voice mode for inscriptions is 3200 — cleaner speech at a
# still-lean ~400 B/s. The mode is stored in codec_meta so any decoder recovers it.
C2_DEFAULT_MODE = 3200
C2_FRAME = 320      # legacy 700C frame size (kept for back-reference)
C2_BPF = 4          # legacy 700C bytes/frame
C2_MODE = 700       # legacy 700C mode


def _get_c2(mode=C2_DEFAULT_MODE):
    """Lazy-import the Codec2 binding (so the 0x00/0x01 codecs still work without
    libcodec2). Returns (codec, samples_per_frame, bytes_per_frame) for `mode`."""
    import pycodec2
...
def encode_sound_codec2(audio_samples, *, mode=C2_DEFAULT_MODE, title="",
    ...
    """Encode mono 8 kHz audio with Codec2 (`mode`, default 3200) into a canonical
    SOUND container (type 0x07, codec 0x02). Needs libcodec2 + pycodec2. Returns
    (header_bytes, body_bytes); codec_meta = mode:u16 + n_frames:u32 (big-endian)."""
    import struct
    np = _np()
    x = np.asarray(audio_samples, dtype=np.float32)
    if x.ndim != 1:
...
def decode_voice_c2(header_bytes, body_bytes):
    ...
    if len(meta) >= 6:                          # mode:u16 + n_frames:u32
        ...
        mode, n_frames = 700, sound.unpack_frames_meta(meta)
    body_bytes = rec["body"]
