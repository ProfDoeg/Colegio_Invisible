#!/usr/bin/env python3
"""Run the twelve tribes dossiers unattended: N at a time, skipping any that exist, committing
and pushing each one as it lands (exact paths, both remotes), logging to
working/tribes/batch_progress.log. Same driver as working/bode/run_batch.py.

    python3 working/tribes/run_batch.py [concurrency]
"""
...
RUNNER = os.path.join(REPO, "working", "journeys", "atlas_tools", "codex_dossier_tribes.py")
...
def land(slug, name):
    rel = f"{REL}/{slug}.dossier.md"
    with git_lock:
        git("add", rel)
        git("commit", "-q", "-m", f"tribes: {name} dossier", "-m",
            "Codex-researched from the tribe prompt and the per-tribe brief (working/tribes). "
