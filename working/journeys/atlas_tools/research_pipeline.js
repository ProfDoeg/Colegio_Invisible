...
- NEVER use Edit, Write, or NotebookEdit on any file under the atlas directory, and never
  ...
  atlas_tools/_scratch_${SLUG}_<yourlens>.py helper -- the slug first, always:
  two subjects running concurrently in the same checkout share this directory,
  and a bare _scratch_<yourlens>.py collides and gets silently overwritten
  mid-run by the other subject's pass.
...
- NEVER use a bash heredoc (<<PY, <<'PY', <<EOF). For any python beyond a trivial
  ...
  ${A.dir}/atlas_tools/_scratch_${SLUG}_<yourlens>.py, THEN run
  python3 ${A.dir}/atlas_tools/_scratch_${SLUG}_<yourlens>.py. Delete is unnecessary; leave scratch files in place.
  Slug first in the filename, always -- see READONLY above for why.
...
// ---- Phase 3: Write (opus) --------------------------------------------------
const writeResult = await agent(`Write the two atlas artifacts for ${NAME} (slug: ${SLUG}) from this verified research pool:\n${JSON.stringify(pool)}\n\nFirst READ two existing exemplars for form: ${DIR}/abdelkader.journey.json (schema and campa register) and ${DIR}/bourlemont_roster.md (report method). Then Write BOTH files:\n\n1. ${DIR}/${SLUG}.report.md - the research report: sections by life phase, every claim tagged [A source] or [R], contradictions flagged not resolved, honest gaps, a Sources section listing what was reachable and what was not. 8-20 KB.\n\n2. ${DIR}/${SLUG}.journey.json - top-level keys traveler, title, years, register (exactly "national mythology: the canon is true"), calendar ("julian" for pre-1582 lives, "gregorian" after), segments: 5-9 named segments, each with stops in travel order. Each stop: name (Place, the event - colon form, no dash), lat, lng, date (full ISO; default unknown month/day to 01), date_confidence (the scholarly hedge lives HERE, never in prose), campa (450-650 chars, the narrative), quote + quote_source (only where the verified pool has a real one), sources (list), suggested_refs (list). 25-45 stops. Inherit canonical pins EXACTLY where the interlock lens found them (byte-identical, e.g. Kaaba 21.4225, 39.8262). Name another existing atlas traveler in a campa ONLY where ${NAME} themself has a direct or indirect relation to that person (met them, corresponded, was influenced by/influenced them, family, rivalry, cited them, wrote substantively about them even one-directionally across centuries -- a commentator writing on a historical or biblical figure is a real link even though the reverse could never be true, etc.) -- never merely because both happened to pass through the same place or time. Shared geography alone is not a reason to name-drop; the atlas's own map already shows co-location to a viewer.\n\nA STOP BELONGS TO WHOEVER WAS ACTUALLY THERE. If ${NAME}'s only connection to a place/object is that someone ELSE (a later commentator, admirer, or scholar) traveled there or engaged with it, that is THAT OTHER PERSON's stop, on their own journey file, not ${NAME}'s. Cite the other person's engagement as a SOURCE on ${NAME}'s own stop for that place/object (e.g. an artist's stop for the object they made, citing a later critic's essay about it in suggested_refs/sources) -- never stage the other person's visit as its own scene inside ${NAME}'s journey. This mirrors the interlock rule above: a one-directional relation is real and citable, but it still belongs physically wherever the traveling happened.\n\nNO AFTERLIFE SEGMENT (Anthony, 2026-09-11: "Any afterlife takes place in other people's journeys when they write about or practice" the subject's art). A journey ends with its subject. Later readers, editors, disciples, imitators and scholars are NOT stops on ${NAME}'s file; their engagement belongs on their own journeys if they are atlas subjects, and on ${NAME}'s file only as suggested_refs or sources attached to the stop for the work or place they engaged with. The afterlife lens's material is for those refs and for the report, not for scenes. The one thing that may follow the death as a stop is ${NAME}'s own body and its fate (burial, translation of relics, a tomb opened), because ${NAME} was there.\n\nTHE APPARATUS EXCEPTION. READ ${DIR}/EXCEPTIONS.md before you drop any relation for lack of biographical attestation. It names the subjects whose theoretical framework MAY be applied BACKWARD, as stops on their OWN file, to past events they never themselves discussed. If ${NAME} is listed there, such stops are licensed: put them at ${NAME}'s own pin, date them only to order the segment, and state the interpretive status plainly in date_confidence along with the primary sources for the material itself. The template is j_l_austin.journey.json, segment "The Formula That Does Not Describe: Magic and Prophecy", whose Briar Rose stops sit at Austin's Oxford pin and say in their own confidence fields that Austin never discussed the tale. The exception NEVER licenses a forward reference: write nothing into the earlier subject's file, and do not apply a dead theorist's framework to events that postdate them. If you find a relation not rooted in ${NAME}'s own life and ${NAME} is NOT on that list, do NOT silently delete it. Keep it, and flag it in the report under a heading "Apparatus relations for the operator", saying which direction you caught, backward or forward, so the human operator can decide whether to add ${NAME} to the list. Silent deletion is what cost john_searle.journey.json both its Merlin material and its LLM material on its first pass.\n\n${HOUSE}\n\nReturn JSON: {report_path, journey_path, stops, segments, interlocks: [slugs named in campa]}.`,
